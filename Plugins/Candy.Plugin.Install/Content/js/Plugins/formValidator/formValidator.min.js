    ...
    $.formValidator =
{
    ...
    setTipState: function (elem, showclass, showmsg) {
        ...
        clearTimeout(elem.validatetimer);
        ...
        if (initConfig.mode == "SingleTip") {
            ...
            var setting = elem.settings[0],
				relativeTip = $("#" + setting.relativeID),
				offset = relativeTip.offset(),
				top = offset.top + setting.tipCss.top;

            left = relativeTip.outerWidth() + offset.left - tip.outerWidth() + setting.tipCss.left;

            tip.css({ top: top, left: left });

            if (showclass == "onCorrect") {
                // 成功提示在1500ms后自动隐藏
                elem.validatetimer = setTimeout(function () {
                    $.formValidator.setTipState(elem, "onShow", null);
                }, 1500)
            }
        }
    },
        ...
        return this.each(function (e) {
            ...
            if (initConfig.mode == "AutoTip" || initConfig.mode == "FixTip") {
                ...
                if (initConfig.mode == "AutoTip" && tipDiv.length == 0) {
                    ...
                    // setting_temp.tipCss.left = $("#"+relativeID ).outerWidth() + offset.left - tipDiv.outerWidth() + setting_temp.tipCss.left;
...
var regexEnum =
{
    ...
    money: "^(-)?(([1-9]{1}\d*)|([0]{1}))(\.(\d){1,2})?$", //精确到小数点两位的金额格式  //addByMRZ 2012年3月30日
    money1: "^([0-9]+|[0-9]{1,3}(,[0-9]{3})*)(.[0-9]{1,2})?$", // 金额验证，非负整数，精确到小数点两位
    email: "^\\w+((-\\w+)|(\\.\\w+))*\\@[A-Za-z0-9]+((\\.|-)[A-Za-z0-9]+)*\\.[A-Za-z0-9]+$", //邮件
    color: "^[a-fA-F0-9]{6}$", 			//颜色
    url: "^(http[s]?:\\/\\/)?[\\w\\-_]+(\\.[\\w\\-_]+)+([\\w\\-\\.,@?^=%&amp;:/~\\+#]*[\\w\\-\\@?^=%&amp;/~\\+#])?$", //url
    local: "^(http[s]?:\\/\\/)?localhost([\\w\-\\.,@?^=%&amp;:/~\\+#]*[\\w\\-\\@?^=%&amp;/~\\+#])?",
    chinese: "^[\\u4E00-\\u9FA5\\uF900-\\uFA2D]+$", 				//仅中文
    ...
    legalMobile: "^((13[4-9])|(15[012789])|147|182|187|188)[0-9]{8}$"  //合法的11位手机号
}
